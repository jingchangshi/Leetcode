...
public:
    ...
    TreeNode* buildTreeFromArray(vector<int>& inorder, vector<int>& postorder,
        ...
        auto root_it = find(inorder.begin() + in_beg, inorder.begin() + in_end, root_val);
        // int left_end = root_it - &inorder[in_beg];
        int left_end = root_it - inorder.begin();
    ...
    void dfsInorder(TreeNode *root, vector<vector<int>>& vec, int m) {
      if (root == nullptr) {
        return;
      }
      dfsInorder(root->left, vec, m+1);
      if (vec.size() <= m) {
        vec.resize(m+1);
        vec[m].push_back(root->val);
      } else {
        vec[m].push_back(root->val);
      }
      dfsInorder(root->right, vec, m+1);
    }

    vector<vector<int>> levelOrder(TreeNode* root) {
      vector<vector<int>> vec;
      dfsInorder(root, vec, 0);
      return vec;
    }
};
